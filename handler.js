...
module.exports = async (client, m) => {
   try {
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', 'Bot sudah kadaluwarsa di grup ini, bot segera minggat...', null, {
      ...
      if (users && (new Date * 1) >= users.expired && users.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', 'Premium plan kamu sudah habis, silahkan tambah lagi :v')).then(async () => {
            users.premium = false
            users.expired = 0
         })
      }
      ...
      if (m.isGroup && !m.isBot && users && users.afk > -1) {
         client.reply(m.chat, `Kamu kembali online lagi setelah offline untuk : ${Func.texted('bold', Func.toTime(new Date - users.afk))}\n\n• ${Func.texted('bold', 'Alasan')}: ${users.afkReason ? users.afkReason : '-'}`, m)
         users.afk = -1
         users.afkReason = ''
      }
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && commands.includes((body.split` ` [0]).toLowerCase())) {
         ...
         if (!commands.includes(command) && matcher.length > 0 && !setting.self) {
            if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `Command tidak terdaftar, mungkin maksud kamu :\n\n${matcher.map(v => '➠ *' + (isPrefix ? isPrefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
         }
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'confess', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `Saat ini Owner sedang mengaktifkan Mode Group\njadi bot hanya bisa digunakan di dalam grup, silahkan join ke grup di bawah ini\njika ingin sewa ke grup silahkan ke *.owner*`, m, {
               largeThumb: true,
            ...
            if (cmd.limit && users.limit < 1) {
               return client.reply(m.chat, `Limit kamu dah habis*`, m).then(() => users.premium = false)
               continue
            }
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, Func.texted('bold', `Limit kamu ga cukup cuy.`), m)
                  continue
               }
            }
      ...
      //console.log(e)
      if (!m.fromMe) m.reply(global.forwads, Func.jsonFormat(e))
   }
}
