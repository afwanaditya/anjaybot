...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         if (!m.quoted) return client.reply(m.chat, Func.texted('bold', `cuma bisa ke image, video belum support.`), m)
         if (m.quoted.mimetype != 'image/webp') return client.reply(m.chat, Func.texted('bold', `cuma bisa ke image, mp4 belum support.`), m)
