...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         if (!text) return client.reply(m.chat, Func.example(isPrefix, command, 'adit kece!'), m)
         if (text.length > 30) return client.reply(m.chat, Func.texted('bold', `Kepanjangan coy, maksimal 30 character.`), m)
         ...
         console.log(e)
         client.reply(m.chat, Func.texted('bold', `gagal membuat sticker.`), m)
      }
   },
   error: false,
   limit: true,
